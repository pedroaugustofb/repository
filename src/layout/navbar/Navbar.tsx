...
//Navbar Styles
import { NavbarContainer, NavList, NavItem, LanguageButton, MobileStruct, MobileModal, MobileButton, MobileList, Bar } from "./style";

//components
const Fallback = lazy(() =>
  import("../../components/Fallback").then((module) => {
        ...
        <MobileButton onClick={() => setModal((prev) => !prev)}>
          ...
          <MobileModal active={Modal} backgroundColor={backgroundColor}>
            <MobileList active={Modal}>
              <NavItem to="/" home />
              <NavItem
                text={language === "PT-BR" ? "Portfólio" : "Portfolio"}
                to="/portfolio"
              />
              <NavItem
                text={language === "PT-BR" ? "Currículo" : "Curriculum"}
                to="/cv"
              />
            </MobileList>
          </MobileModal>
